...
%year = 2015; month = 11;
%%%%%%%%%%%%%%%%%%%%%%%%%
%filename_ts = sprintf('OMAPS_%d_%d_tse.nc', year, month); 
%filename_uv = sprintf('OMAPS_%d_%d_uv.nc', year, month); 
filename_ts = ['OMAPS_' num2str(year) '_' num2str(month, '%.2d') '_tse.nc']; 
filename_uv = ['OMAPS_' num2str(year) '_' num2str(month, '%.2d') '_uv.nc'];
filename_hf = ['../CFSv2/CFSv2_' num2str(year) '_heatflux.nc'];
...
if h == 60; 
  ...
  %mean(xt(iys,ixw-1:ixw)) %xw
  %mean(xt(iys,ixe-1:ixe)) %xe
  tw = squeeze(mean(cdf_ts{'temp'}(:,nz-9:nz,iys:iyn-1,ixw-1:ixw),4)); 
  te = squeeze(mean(cdf_ts{'temp'}(:,nz-9:nz,iys:iyn-1,ixe-1:ixe),4)); 
  %mean(yt(iys-1:iys,ixw)) %ys
  %mean(yt(iyn-1:iyn,ixw)) %yn
  ts = squeeze(mean(cdf_ts{'temp'}(:,nz-9:nz,iys-1:iys,ixw:ixe-1),3)); 
...
%
  ...
  tmp = squeeze(mean(sfc_hflux(which,:,:),1)); sfc_hflux_avg(it,1) = mean(tmp(:)); % Daily and spatial average
%
% END EDITS
%
  for iz = 1:length(DZ)
