...
function f = calc_temp_vol_avg(xw,xe,yn,ys,year,month, header_OMAPS, data_shf);
...
%h = 60; izh = 10; %depth of mixed layer
h = 100; izh = 14;
%h = 150; izh = 19;
%h = 200; izh = 24;
...
%
  ...
  tmp = squeeze(mean(sfc_hflux(which,:,:),1)); sfc_hflux_avg(it,1) = sum(sum(tmp.*dA_hf))/sum(sum(dA_hf)); % Daily and spatial average
%
% END EDITS
%
  for iz = 1:length(DZ)
